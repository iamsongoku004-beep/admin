...
async function adminLoadTeam() {
  ...
  try {
    ...
    if (data[ABSENT_GIST_KEY]) {
      localStorage.setItem(getAbsentKey(), JSON.stringify(data[ABSENT_GIST_KEY]));
    }
    renderTeamDashboard();
  initTiltCards();
...
// ===== ABSENT STATUS SYNC VIA GIST =====
async function syncAbsentToGist(absentList) {
  if (!githubToken || !gistId) return;
  try {
    let data = await readGist();
    if (!data) data = { members: {} };
    data[ABSENT_GIST_KEY] = absentList;
    await writeGist(data);
  } catch (e) {
    console.error('Sync absent error:', e);
  }
}

async function getAbsentFromGist() {
  if (!githubToken || !gistId) return null;
  try {
    let data = await readGist();
    if (data && data[ABSENT_GIST_KEY]) {
      return data[ABSENT_GIST_KEY];
    }
  } catch (e) {
    console.error('Get absent error:', e);
  }
  return null;
}

// ===== ABSENT STATUS MANAGEMENT =====
// NOTE: must be a function, not a constant, because teamId is '' when this
// file first loads and only gets set later inside adminLoadTeam(). A constant
// here would freeze the key to 'callTracker_absent_' forever and silently
// read/write the wrong localStorage entry for every team.
function getAbsentKey() {
  return 'callTracker_absent_' + teamId;
}
const ABSENT_GIST_KEY = 'absent_members';

function getAbsentMembers() {
  const saved = localStorage.getItem(getAbsentKey());
  return saved ? JSON.parse(saved) : [];
}
...
function setAbsentStatus(name, isAbsent) {
  ...
  localStorage.setItem(getAbsentKey(), JSON.stringify(absent));
  syncAbsentToGist(absent);
  renderTeamDashboard();
  initTiltCards();
}
...
function clearAllAbsent() {
  localStorage.removeItem(getAbsentKey());
  syncAbsentToGist([]);
  renderTeamDashboard();
  initTiltCards();
}
